...
function loadCategorias() {
  ...
  `)
  const colRef = db.collection("categorias")
  .where("tipo", "==", 0)
  .orderBy("nombre", "asc");
  colRef.get()
    .then((querySnapshot) => {
...
function loadDataNoticia() {
  const noticiaID = getQueryVariable("id").trim();
  const noticiaRef = db.collection("noticias").doc(noticiaID);

  noticiaRef.get().then((doc) => {
    if (doc.exists) {
      const data = doc.data();
      $("#titulo-noticia").val(data.titulo);
      $("#subtitulo-noticia").val(data.subtitulo ?? "");
      $("#contenido-noticia").val(data.contenido ?? "");

      if (data.categoria && data.categoria.id) {
        setTimeout(function () {
          $("#select-categoria").val([data.categoria.id]);
        }, 100);
      }

      if (data.imagenes && data.imagenes[0] && data.imagenes[0].toString().length) {
        for (let i = 0; i < data.imagenes.length; i++) {
          if (i <= 5) {
            $("#img-noticia" + (i + 1).toString()).attr("src", data.imagenes[i]);
            $("#img-noticia" + (i + 1).toString()).next().next().css({ "visibility": "visible" });
            $("#img-noticia" + (i + 1).toString()).next().attr("x-data", data.imagenes[i]);
          }
        }
      }
    }
...
$(document).ready(function () {
  ...
  $("#btn-subir-img").on("click", function () {
    ...
    const noticiaID = getQueryVariable("id").trim();
    const ref = storageRef.child('noticias/' + noticiaID + "_" + new Date().getTime() + '.jpg');
...
function guardarNoticia() {
  ...
  if (!titulo.length) {
    ...
    db.collection("noticias").doc(noticiaID).update({
      titulo: titulo,
      subtitulo: subtitulo,
      categoria: {
        nombre: categoria,
      ...
      messageNoticiaEditada();
    }).catch((error) => {
      errorEditarNoticia(error);
    });
  }
}

function errorEditarNoticia(error) {
  $("#btn-guardar-noticia").attr("disabled", false);
  swal("Ocurrió un error al editar el producto! Intentá de nuevo", "", "error");
  console.log(error);
}

function messageNoticiaEditada() {
  $("#contenido").html(`
  <div id="outer" class="container d-flex align-items-center justify-content-center" style="height:550px">
    <div id="inner">
        ...
        <h2 class="text-center mt-3 mb-3">¡Editaste la Noticia correctamente!</h2>
